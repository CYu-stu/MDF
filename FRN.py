...
class FRN(nn.Module):
    ...
    # Woodbury: whether to use the Woodbury Identity as the implementation or not
        ...
        Q = query.view(-1, self.resolution, self.d) #这个步骤重塑了query的形状
        st_Q = Q.permute(0, 2, 1)

        reg_Q = Q.size(1) / Q.size(2)#计算Q的第1维与第2维的比值，生成一个比例因子，用于后续的正则化。
        lam_Q = reg_Q * self.r_Q[0].exp() + 1e-6#这是为了生成一个正则化的权重lam_Q
        rho_Q = self.r_Q[1].exp()#用于后续的缩放操作
